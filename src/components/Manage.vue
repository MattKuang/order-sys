			...
			<div class="control">
				<div class="add">
					<dl>
						...
						<dd>
							...
							<!-- <input type="text" v-model="newDishes.classify" > -->
								...
								<el-select v-model="newDishes.classify" placeholder="请选择菜品分类">
					...
					<!-- <button @click="addDishes">添加</button> -->
					 <el-button  @click="open();addDishes()">添加</el-button>
				</div>
				<div class="del">
					<dl>
						<dt>删除菜品</dt>
						<dd v-for="(item,index) in totalDishes" :key="index">
							<span>{{item.name}}</span>
...
/* 		befroeRouteEnter(to, from, next){
			next(vm=>{
				console.log(this.$store.state.dishesData);
			});
		}, */
/* 		beforeRouteLeave(to, from, next){
			const answer = confirm('确定要离开管理页面吗？')
			if(answer){
				next();
			}else{
				next(false);
